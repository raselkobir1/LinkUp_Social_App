...
- **Video call** (`VideoCallHub` + `call.service.ts` + `video-call.component.ts`): 1:1 WebRTC works and is e2e-tested. SDP/ICE are relayed as JSON strings. Caller creates the offer on `CallAccepted`; callee answers the offer; ICE candidates are buffered until the remote description is set. The hub is connected app-wide from the shell so incoming calls ring anywhere. Group-call **signaling** exists (`InitiateGroupCall`/`JoinCall`/`LeaveCall` + `GroupCallInitiated`/`ParticipantJoined`/`ParticipantLeft`), but the Angular WebRTC-mesh UI to consume it is not built.
- **`array.Contains(valueType)` breaks EF query translation** on .NET 10 — it binds to `ReadOnlySpan<T>.Contains` which EF can't translate (500). Use `List<T>` for any collection used inside an EF `.Contains(...)` predicate (fixed once in `VideoCallManager.GetActiveCallAsync`).
- **Notifications are generated cross-module**: managers inject `INotificationManager` and call `CreateNotificationAsync`, which persists + broadcasts over `NotificationHub` and **honors per-user `NotificationSettings`** (disabled categories are silently suppressed). Wired triggers: FriendRequest/Accepted (Friend), PostLike (Reaction→reads `PostDbContext` for author), PostComment/CommentReply/Mention (Comment), Mention (Chat), GroupInvite (GroupChat). **NewMessage and VideoCall notification types are intentionally NOT in the feed** (handled by chat unread badge / the call-ring hub).
- **Presence**: `ChatHub.OnConnected/OnDisconnected` persist `ApplicationUser.IsOnline` + `LastSeen` and broadcast `UserOnline`/`UserOffline`. Messages are marked `Delivered` server-side when a recipient is online; `MarkDelivered`/`MarkAsRead` hub methods broadcast `MessageDelivered`/`MessageRead`.
- **@mentions**: `SharedKernel/Helpers/MentionParser` extracts `@username` tokens; Comment + Chat managers resolve them via `UserManager.FindByNameAsync` and raise `Mention` notifications (chat mentions only fire for chat participants).
- **Profile picture/cover**: `POST /profile/picture` + `/profile/cover` upload via `IMediaService` and write the URL to **both** `UserProfile` AND `ApplicationUser` (friends/posts/chat DTOs read the avatar from `ApplicationUser`, so keep them in sync). Images are compressed on ingest (`q_auto`/`f_auto`, 1600px cap) and get a thumbnail.
- **Block user** (`Friend` module, `BlockedUsers` table): blocking severs friendship + cancels pending requests; enforced in `SendRequestAsync` and `ChatManager.GetOrCreateDirectChatAsync` (Chat→Friend ref). `IsBlockedAsync` checks both directions.

## Known gaps (not yet implemented)
Frontend wiring lags the backend in several places: several unwired comment/post action buttons (reply/like/edit/delete, share, edit post), group-chat management UI, chat attachments/emoji UI, email-verification frontend, and the **group-video mesh UI** (signaling exists). Backend voice-message accepts `MessageType.Voice` + attachment but there's no recorder UI. Call-history UI not built (REST endpoint exists). Notification feed has no NewMessage entries by design.
